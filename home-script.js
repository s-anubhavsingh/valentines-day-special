...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (urlParams.has('p')) {
        // SHORT URL - load from KV backend
        loadFromBackend(urlParams.get('p'));
    } else if (urlParams.has('s')) {
        // OLD long URL format - still supported for backwards compatibility
...
// Load page data from KV backend using short ID
...
async function generateShareableLink() {
    ...
    btn.textContent = 'Saving your page... 💖';
    ...
    // Build data object for KV storage
    const pageData = {
        sender,
        receiver,
...
async function uploadAllPhotos() {
    const photosToUpload = window.uploadedPhotos.filter(p => !p.imageId);
    ...
    const uploadPromises = window.uploadedPhotos.map(async (photo, index) => {
        if (photo.imageId) return photo; // Already uploaded
