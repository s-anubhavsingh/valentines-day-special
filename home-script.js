...
async function uploadAllPhotos() {
    ...
    const status = document.getElementById('upload-status');
    if (!status) {
        alert('Error: Upload status element not found!');
        return window.uploadedPhotos;
    }
    
    status.textContent = `⏳ Uploading ${photosToUpload.length} photo${photosToUpload.length > 1 ? 's' : ''}...`;
    ...
    const uploadPromises = window.uploadedPhotos.map(async (photo, index) => {
        ...
        try {
            ...
            if (data.success && data.url) {
                ...
                alert(`Failed to upload photo ${index + 1}: ${data.error || 'Unknown error'}`);
                failCount++;
            }
        } catch (error) {
            console.error('Upload error for photo', index, error);
            alert(`Network error uploading photo ${index + 1}: ${error.message}`);
            failCount++;
        }
        return photo;
    });
    ...
    if (failCount > 0) {
        status.textContent += ` ⚠️ ${failCount} failed`;
